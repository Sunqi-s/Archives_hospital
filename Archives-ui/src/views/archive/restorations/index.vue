...
<!--          size="mini"-->
<!--          @click="handleAdd"-->
<!--          v-hasPermi="['archive:restorations:add']"-->
<!--        >新增</el-button>-->
<!--      </el-col>-->
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
...
<!--          :disabled="single"-->
<!--          @click="handleUpdate"-->
<!--          v-hasPermi="['archive:restorations:edit']"-->
<!--        >修改</el-button>-->
<!--      </el-col>-->
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
...
<!--          :disabled="multiple"-->
<!--          @click="handleDelete"-->
<!--          v-hasPermi="['archive:restorations:remove']"-->
<!--        >删除</el-button>-->
<!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['archive:restorations:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="restorationsList" @selection-change="handleSelectionChange">
...
<script>
import { listRestorations, getRestorations, delRestorations, addRestorations, updateRestorations } from "@/api/archive/restorations";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listRestorations(this.queryParams).then(response => {
        this.restorationsList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getRestorations(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateRestorations(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRestorations(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除归还编号为"' + ids + '"的数据项？').then(function() {
        return delRestorations(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('archive/restorations/export', {
        ...this.queryParams
      }, `restorations_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
